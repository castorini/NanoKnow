#!/usr/bin/env python3
"""
Generate Anserini-compatible topics and qrels files for NanoKnow SQuAD.

This script intentionally reads question text from the Stage 2 pickle instead of
trying to recover it from the CSV-like qrels file, since questions and answers
...
DEFAULT_SUPPORTED_QRELS = REPO_ROOT / "qrels" / "squad_supported_full.txt"
DEFAULT_PICKLE = Path(
    "/u201/njedidi/past_projects/nanochat_tests_nour/"
    "squad_fineweb_eval_val_stage2.pkl"
)
DEFAULT_ANSWERS = SCRIPT_DIR / "answers.nanoknow-squad.jsonl"
DEFAULT_SUPPORTED_TOPICS = SCRIPT_DIR / "topics.nanoknow-squad.supported.tsv"
DEFAULT_UNSUPPORTED_TOPICS = SCRIPT_DIR / "topics.nanoknow-squad.unsupported.tsv"
DEFAULT_TREC_QRELS = SCRIPT_DIR / "qrels.nanoknow-squad.supported.txt"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build SQuAD-only Anserini topics/qrels files from NanoKnow inputs."
    )
    parser.add_argument(
        "--supported-qrels",
        type=Path,
        default=DEFAULT_SUPPORTED_QRELS,
        help="Path to squad_supported_full.txt",
    )
    parser.add_argument(
        "--pickle-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Where to write answers.nanoknow-squad.jsonl",
    )
    parser.add_argument(
        "--supported-topics-out",
        type=Path,
        default=DEFAULT_SUPPORTED_TOPICS,
        help="Where to write topics.nanoknow-squad.supported.tsv",
    )
    parser.add_argument(
        "--unsupported-topics-out",
        type=Path,
        default=DEFAULT_UNSUPPORTED_TOPICS,
        help="Where to write topics.nanoknow-squad.unsupported.tsv",
    )
    parser.add_argument(
        "--qrels-out",
        type=Path,
        default=DEFAULT_TREC_QRELS,
        help="Where to write qrels.nanoknow-squad.txt",
    )
...
def assert_question_matches_line(
    qid: str, line: str, questions: dict[str, str], *, exact_question_match: bool
) -> None:
...
def iter_supported_pairs(supported_qrels: Path, questions: dict[str, str]):
    with supported_qrels.open("r", encoding="utf-8") as handle:
        for raw_line in handle:
            ...
            assert_question_matches_line(qid, line, questions, exact_question_match=True)
            yield qid, docid
...
def verify_topics_file(questions: dict[str, str], qids_in_order: list[str], topics_path: Path) -> int:
    ...
    assert observed_qids == expected_qids, (
        "Generated topics qids do not match the expected unique qids/order derived from "
        "squad_supported_full.txt"
    )
...
def verify_qrels_file(qid_docids: list[tuple[str, str]], qrels_path: Path) -> int:
    ...
    assert observed_pairs == qid_docids, (
        "Generated qrels do not exactly match the qid/docid pairs from squad_supported_full.txt"
    )
